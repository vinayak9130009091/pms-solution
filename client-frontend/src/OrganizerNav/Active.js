...
export default function DataTable() {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:7600/organizer-account-wise/account/66ac94cac1fd3a0ef310a550')
      .then((response) => response.json())
      .then((data) => {
        console.log('API Response:', data); 
        
        // Assuming the correct array is at `data.organizers`
        const fetchedRows = (Array.isArray(data.organizers) ? data.organizers : []).map((item, index) => ({
          id: index + 1,
          name: item.organizerName,
          lastUpdate: new Date(item.createdAt).toLocaleTimeString(),
          status: 'Pending', 
          seal: '', 
        }));
